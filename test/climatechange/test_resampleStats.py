...
#           assert_almost_equal([[]], compileStats(emptyArray))
    ...
    def test_resampled_depths_by_years(self):            
        ...
        result = resampled_depths_by_years(input_test_zeros_and_numbers,headers,index)
        assert_frame_equal(expected_result, result)
    
    def test_create_stats_headers(self):
    ...
    def test_compile_stats_by_year(self):
        expected_result = load_csv(os.path.join('csv_files', 'output_test_zeros_and_numbers.csv')) 
        result = get_compiled_stats_by_year(input_test_zeros_and_numbers, headers,inc_amt)
        assert_frame_equal(expected_result, result[0][0].df)
    ...
    def test_empty_rows(self):
        ...
        with warnings.catch_warnings():
            warnings.simplefilter("ignore", category=RuntimeWarning)
            result = get_compiled_stats_by_year(input_test_zeros, headers_zeros,1)
        assert_frame_equal(expected_result, result[0][0].df)
